...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_DEV_ATTACHED :	//0x01					
    
    phost->usr_cb->DeviceAttached();			//检测USB设备插入
    ...
    if ( HCD_ResetPort(pdev) == 0)
    {
      ...
      */
      ...
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);	//检测到从机速度
        
      /* Open Control pipes */					//打开控制管道
  ...
  case HOST_CTRL_XFER:
    ...
    USBH_HandleControl(pdev, phost);    			//0x07
    break;
