...
u8 mf_read(u32 len)
{
	...
	g_iap.APP_RX_len = 0;						//清零2Kbuff指针
	
	for(i=0;i<len/512;i++)						//大于512字节
	{
		res=f_read(file,fatfs_buff,512,&br);
		if(res)		
		{
			//printf("Read Error:%d\r\n",res);
			break;
		...
		else		
		{
			...
			for(t=0;t<br;t++)
			{
				g_iap.RX_BUFF[g_iap.APP_RX_len] = fatfs_buff[t]; 
				g_iap.APP_RX_len++;
			}
	...
	if(len%512)									//小于512字节
	{
		res=f_read(file,fatfs_buff,len%512,&br);
		if(res)	//读数据出错了
		{
			//printf("\r\nRead Error:%d\r\n",res);   
		}
...
{
 	TCHAR* rbuf;
	rbuf=f_gets((TCHAR*)fatfs_buff,size,file);
