...
void AnalyzeRecieve(u8 *RX_buff,u16 buff_len)                              //分析并生成响应报文
{
    uint16_t _crc;
    uint8_t a1, a2;
    _crc = CalcCrc(RX_buff, (buff_len - 2)); //计算校验CRC
    a1 = _crc & 0xff;                      //CRC低字节
    a2 = (_crc >> 8) & 0xff;               //CRC高字节
    if(a1 == RX_buff[buff_len - 2] && a2 == RX_buff[buff_len - 1]) //校验正确
    {
        switch(RX_buff[1])
        {
        case 0x01:
...
void  send_comm( void )		//DMA发送
{
    Uart4_Start_DMA_Tx(sendlength);
}
//void  send_comm( void )			//串口发送
//{
//	GPIO_SetBits(GPIOA,GPIO_Pin_2);							//高电平发送
//	USART_OUT(UART4,(char *)sendBuf,sendlength);
//	GPIO_ResetBits(GPIOA,GPIO_Pin_2);						//低电平接收	
//	memset(receBuf,0,sizeof(receBuf));						//接收数组清零
//	memset(sendBuf,0,sizeof(sendBuf));						//接收数组清零
//}
...
/*分析06功能码报文，产生响应报文*/
void Recirve_06()//单字节写入				//PLC_Data
{
...
/*分析10功能码报文，产生响应报文*/
void Recirve_10()//接收到数据				//PLC_Data 多个
{
